...
4. 单人录音不要写「说话人1」。多人讨论的摘要、大纲、待办和决策也不要使用机械编号；只有「智能章节」需要区分观点时才保留「说话人1/说话人2」。
...
def speaker_policy_text(transcript: str) -> str:
    ...
    return (
        "这是一段多人讨论。摘要、大纲、待办和决策不要写「说话人1/2」这类机械编号，"
        "有真实身份就用真实身份，否则用客观陈述；只有「智能章节」需要区分观点时"
        "才可以保留「说话人1」「说话人2」。不要出现 speaker_1 这类英文标签。"
    )
...
_SPEAKER_NUMBER = re.compile(r"(?:说话人|发言人)\s*(\d+)|speaker_(\d+)", re.IGNORECASE)
_PROTECTED_SPEAKER_SECTIONS = {
    "智能章节",
    "金句时刻",
    "完整文字稿",
    "文字稿",
...
def neutralize_summary_speaker_numbers(markdown: str) -> str:
    """自然化总结区的说话人编号，同时保留章节叙述和原话证据。"""
    ...
    for index, line in enumerate(lines):
        ...
        if not in_code_fence and section not in _PROTECTED_SPEAKER_SECTIONS:
            target_indexes.append(index)

    ordered: list[str] = []
    for index in target_indexes:
        for match in _SPEAKER_NUMBER.finditer(lines[index]):
    ...
    if len(ordered) == 1:
        aliases = {ordered[0]: "有发言人"}
    else:
        labels = ["一方", "另一方", "第三位发言人", "第四位发言人", "第五位发言人"]
        aliases = {
            number: (labels[index] if index < len(labels) else f"第{index + 1}位发言人")
            for index, number in enumerate(ordered)
        }

    def replace(match: re.Match[str]) -> str:
        number = match.group(1) or match.group(2)
        return aliases.get(number, match.group(0))

    duplicate_suffix = re.compile(
        r"(一方|另一方|第[一二三四五六七八九十\d]+位发言人|有发言人)方"
    )
    for index in target_indexes:
        lines[index] = _SPEAKER_NUMBER.sub(replace, lines[index])
        lines[index] = duplicate_suffix.sub(r"\1", lines[index])
    return "".join(lines)
...
def generate_minutes(task_dir: Path, config: dict[str, Any]) -> bool:
    ...
    summary = neutralize_summary_speaker_numbers(summary)
