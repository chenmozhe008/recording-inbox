...
作者日常用的是一个自建的录音 App（灵动岛计时、分段保存、录音标记、直传飞书），**暂未随本仓库开源**——一是它和作者的私人流程耦合较深，二是苹果对个人开发者签名的 App 只给 7 天有效期，没法直接分发安装包，对多数人实用价值有限。

本仓库的链路**完全不依赖**这个 App：方式一/方式二就是完整体验。如果后续把 App 整理开源，会在 README 更新说明。
