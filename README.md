...
| 手机和电脑统一入口 | iPhone、Android、微信下载音频和电脑文件都进入同一个飞书“录音收件箱” |
...
电脑临时关机没有关系：录音留在飞书“录音收件箱”，开机后自动接着处理。电脑关机时本地程序无法主动发“离线通知”，真正接手、完成或失败后才会通知。
...
| 录音在哪里 | 推荐上传方法 |
|---|---|
| 电脑、微信或网盘 | 直接拖进飞书“录音收件箱”，最简单 |
| iPhone 语音备忘录 | 存到“文件” → 飞书“云盘” → 录音收件箱 → 右下角“上传” → 选择文件 |
| Android 系统录音机 | 飞书“云盘” → 录音收件箱 → 右下角“上传” → 选择录音 |

第一次找到“录音收件箱”后，可以把它加入飞书“收藏”，以后少找一层目录；不收藏也不影响上传。完整步骤见 [手机和电脑上传教程](docs/upload-from-phone.md)。
...
- 新安装使用仓库实跑验证过的 FunASR/ModelScope 核心版本；环境自检会显示实际版本，避免依赖静默升级后难以排查。
- 每条录音有独立任务包和状态，不靠内存记进度。
- 电脑关机或进程中断后，会从转写、纪要或发布阶段继续。
- 上一轮真的还在运行时，新一轮会跳过，避免重复转写。
- 断电留下的死锁会在开机后立即清理，不会再等待数小时。
- 同一飞书文件只处理一次；电脑接收到原音频后，会移入“录音收件箱”里的“已接收原始音频”子文件夹。
